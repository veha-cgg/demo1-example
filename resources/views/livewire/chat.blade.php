        ...
        <div class="flex-1 overflow-y-auto">
            @forelse($users as $user)
                <button
                    ...
                    class="w-full p-4 text-left hover:bg-gray-100 transition-colors {{ $selectedUserId == $user->id ? 'bg-blue-50 border-l-4 border-blue-500' : '' }}"
                >
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50" id="messages-container">
                @forelse($messages as $message)
                    <div class="flex {{ $message->send_id == Auth::id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-xs lg:max-w-md px-4 py-2 rounded-lg {{ $message->send_id == Auth::id() ? 'bg-blue-500 text-white' : 'bg-white text-gray-900 border border-gray-200' }}">
                            <p class="text-sm">{{ $message->message }}</p>
                            <p class="text-xs mt-1 {{ $message->send_id == Auth::id() ? 'text-blue-100' : 'text-gray-500' }}">
        ...
        @else
            ...
            <div class="flex-1 flex items-center justify-center bg-gray-50">
                <div class="text-center">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Select a user from the list to start chatting</p>
                </div>
            </div>
        @endif
    </div>
